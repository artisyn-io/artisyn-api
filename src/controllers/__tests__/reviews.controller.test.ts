...
describe("Reviews Controller", () => {
    ...
    beforeAll(async () => {
        ...
        app.get("/test/reviews", authenticateToken, reviewController.index);
        app.get("/test/reviews/moderation-queue", authenticateToken, reviewController.moderationQueue);
        app.get("/test/reviews/reports", authenticateToken, reviewController.getReports);
        app.get("/test/reviews/aggregation/:targetId", reviewController.aggregation);
        app.get("/test/reviews/:id", authenticateToken, reviewController.show);
        app.post("/test/reviews", authenticateToken, upload.none(), reviewController.create);
        app.put("/test/reviews/:id", authenticateToken, upload.none(), reviewController.update);
        app.delete("/test/reviews/:id", authenticateToken, reviewController.delete);
        app.put("/test/reviews/:id/moderate", authenticateToken, upload.none(), reviewController.moderate);
        app.post("/test/reviews/:id/respond", authenticateToken, upload.none(), reviewController.respond);
        app.put("/test/reviews/:id/respond", authenticateToken, upload.none(), reviewController.updateResponse);
        app.delete("/test/reviews/:id/respond", authenticateToken, reviewController.deleteResponse);
        app.post("/test/reviews/:id/report", authenticateToken, upload.none(), reviewController.report);
        app.put("/test/reviews/reports/:id", authenticateToken, upload.none(), reviewController.resolveReport);
    ...
    describe("POST /reviews/:id/respond", () => {
        it("should allow curator to respond to review", async () => {
            ...
            expect(response.body.data.response.content).toBe("Thank you for your review!");
        });
    ...
    describe("GET /reviews/aggregation/:targetId", () => {
        it("should return rating aggregation for curator", async () => {
            const response = await request(app)
                .get(`/test/reviews/aggregation/${curator.id}`);
        ...
        it("should return 404 for non-existent user", async () => {
            const response = await request(app)
                .get("/test/reviews/aggregation/non-existent-id");
